...
@section('content')
<div class="mb-6"><h1 class="text-2xl font-black text-slate-900">Nouvelle alerte</h1><p class="text-slate-500 text-sm mt-1">Créer une alerte de parc</p></div>
<div class="rounded-2xl border border-slate-200 bg-white shadow-sm p-6">
    <form method="POST" action="{{ route('admin.alertes.store') }}" class="space-y-5">
        @csrf
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <select name="type_alerte" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:ring-primary-500" required>
                    ...
                    @foreach(\App\Models\Alerte::TYPES as $t)
                        <option value="{{ $t }}" @selected(old('type_alerte') == $t)>{{ str_replace('_', ' ', $t) }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="date_echeance" value="{{ old('date_echeance') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:ring-primary-500" required>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="message" rows="3" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:ring-primary-500" required>{{ old('message') }}</textarea>
