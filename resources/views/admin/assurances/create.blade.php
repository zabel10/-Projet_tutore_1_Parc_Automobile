...
@section('content')
<div class="mb-6"><h1 class="text-2xl font-black text-slate-900">Nouvelle assurance</h1><p class="text-slate-500 text-sm mt-1">Ajouter un contrat d'assurance</p></div>
<div class="rounded-2xl border border-slate-200 bg-white shadow-sm p-6">
    <form method="POST" action="{{ route('admin.assurances.store') }}" class="space-y-5">
        @csrf
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input type="text" name="compagnie" value="{{ old('compagnie') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:ring-primary-500" required>
            ...
            <div>
                ...
                <input type="text" name="numero_contrat" value="{{ old('numero_contrat') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:ring-primary-500" required>
            ...
            <div>
                ...
                <input type="date" name="date_debut" value="{{ old('date_debut') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:ring-primary-500" required>
            ...
            <div>
                ...
                <input type="date" name="date_fin" value="{{ old('date_fin') }}" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:ring-primary-500" required>
            ...
            <div class="sm:col-span-2">
                ...
                <select name="type_assurance" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:ring-primary-500" required>
                    @foreach(\App\Models\Assurance::TYPES as $t)
                        <option value="{{ $t }}" @selected(old('type_assurance') == $t)>{{ str_replace('_', ' ', $t) }}</option>
                    @endforeach
                </select>
