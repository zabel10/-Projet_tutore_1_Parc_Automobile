...
@section('content')
<div class="mb-6"><h1 class="text-2xl font-black text-slate-900">Nouvel utilisateur</h1><p class="text-slate-500 text-sm mt-1">Créer un compte administrateur, gestionnaire ou conducteur</p></div>

<div class="rounded-2xl border border-slate-200 bg-white shadow-sm p-6">
    <form method="POST" action="{{ route('admin.utilisateurs.store') }}" class="space-y-5">
        @csrf
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Nom</label>
                <input type="text" name="nom" value="{{ old('nom') }}" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition" required>
            ...
            <div>
                ...
                <input type="text" name="prenom" value="{{ old('prenom') }}" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition" required>
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition" required>
            ...
            <div>
                ...
                <input type="text" name="telephone" value="{{ old('telephone') }}" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition">
                @error('telephone')<p class="text-red-600 text-xs mt-1 font-semibold">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wide">Mot de passe</label>
                <input type="password" name="mot_de_passe" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition" required minlength="8">
            ...
            <div>
                ...
                <input type="password" name="mot_de_passe_confirmation" class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition" required minlength="8">
            </div>
